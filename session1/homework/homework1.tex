...
\paragraph{Part 2 solution.}
    ...
    \[
        \|x + y\|_0 \le \|x\|_0 + \|y\|_0.
    \]
    We iterate over $i \in \{1, \dots, n\}$ and we count the number of non-zero entries in $x + y$ as well as $x$ and $y$ individually.
    For each $i$ there are three cases:
        \begin{enumerate}
            \item $x_i = 0$ and $y_i = 0$. Then also $ x_i + y_i = 0$. Therefore, both sides of the equation increase by 0.
            \item $x_i = 0$ and $y_i \ne 0$. Then also $ x_i + y_i \ne 0$. Therefore, the left side of the equation increases by 1 and the right side of the equation increases by 1 (as one $y_i \ne 0$).
            \item $x_i \ne 0$ and $y_i \ne 0$, but $x_i + y_i = 0$. They are the additive inverse to each other. Then, the left side of the equation increases by 0 and the right side of the equation increases by 2 (as both $x_i \ne 0$ and $y_i \ne 0$).
        \end{enumerate}
    In all three cases, the right side of the equation increases by at least as much as the left side of the equation. Therefore, we have shown that the triangle equality holds for the $\ell_0$ norm.
\end{solution}
...
\begin{solution}
    \paragraph{Part 3 solution.}
    This directly follow by the definition of scalar multiplication. As $\alpha \ne 0$, we iterate through $i \in \{1, \dots, n\}$ and we count the number of non-zero entries in $\alpha x$ as well as $x$ individually.
    If $x_i = 0$, then $\alpha x_i = 0$. Therefore, both sides of the equation increase by 0.
    If $x_i \ne 0$, then $\alpha x_i \ne 0$. Therefore, both sides of the equation increase by 1.

    Therefore, we have shown that the $\ell_0$ norm is zero-homogeneous.
\end{solution}
...
\begin{solution}
    \paragraph{Part 4 solution.}
    If we take the finite field $\field = \mathbb{Z}_5$ and the vector $x = (1, 2) \in \field^2$. Then we have that $x^Tx = 1 \cdot 1 + 2 \cdot 2 = 0$. Therefore, $x$ is orthogonal to itself. (Esentially you have to find a vector where the sum of the squares of the entries is 0).

    To boil it down to the general case, we can always build the nonzero vector of n stacked 1s (i.e. an n-vector x where $x_i=1$ for all $i \in \{1, \dots, n\}$ ) in a finite field $\field = \mathbb{Z}_n$. Then $x^Tx$ = $ n \cdot 1 = 0$. Therefore, $x$ is orthogonal to itself. This is possible because adding 1 to itself $|F|$ times results in 0.
\end{solution}
...
\begin{solution}
    \paragraph{Part 1 solution.}
    We have to show both directions, G linearly independent $\implies d > 0$ and G linearly independent $\impliedby d > 0$.

    \noindent $d>0 \implies$ G is linearly independent: \\
    $d > 0$ implies, that there is no vector except the zero vector where the Matrix vector product with G is equals to 0. Therefore $\nullspace(G)={0}$. Therefore, G is linearly independent.

    \noindent G is linearly independent $ \implies d>0$: \\
    If G is linearly depndent, there are more rows then columns. As G is linearly dependent, no column can be a linear combination of other columns, except the zero vector. $G x$ is a linear combination x of the columns of G. Therefore $\nullspace(G)={0}$. Also, this implies that $G x \ne 0$ for any $x \ne 0$. Therefore, $d > 0$.
\end{solution}
